...
parser.add_argument('--v_max', type=float, help='Maximum velocity.')
...
PATH_BASE = __PATH_ROOT / f"{DIMENSION}D__gamma{GAMMA:.1f}__v_max{V_MAX:.1f}__res{RESOLUTION:03d}/"
if not PATH_BASE.exists():
    raise FileNotFoundError(f"The specified gamma folder does not exist: [{str(PATH_BASE)}]")

path_train_set      = PATH_BASE / f"train.pth"
path_validation_set = PATH_BASE / f"validation.pth"
...
NAME_APPENDIX = f"seed{SEED:03d}"
sys.stdout = open(path_save/f"op_{OPERATOR}__{NAME_APPENDIX}.log", 'w')
...
print(f"* Gamma            >>> {GAMMA}")
print(f"* Maximum speed    >>> {V_MAX} (in each dimension)")
...
for epoch in range(1, 1+NUM_EPOCHS):
    ...
    if epoch==1 or epoch%PERIOD_BACKUP==0:
        torch.save(
            ...
            path_checkpoint / f"op_{OPERATOR}__{NAME_APPENDIX}__epoch{epoch:05d}.pth",
        )
        op.to(device)
...
torch.save(
    ...
    path_save / f"op_{OPERATOR}__{NAME_APPENDIX}.pth",
)
